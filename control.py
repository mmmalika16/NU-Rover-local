...
class ArmController(Node):
        ...
        while rclpy.ok() and attempt < max_attempts:
            if os.path.exists(ARDUINO_PORT):
                try:
                    ...
                    time.sleep(2)  # Allow Arduino to finish reset after serial open
                    self.arduino_serial.reset_input_buffer()   # Clear any stale RX data
                    self.arduino_serial.reset_output_buffer()  # Clear any stale TX data
                    self.arduino_serial.write(b'S\n')
                    self.arduino_serial.flush()                # Force bytes out immediately
                    # Drain any Arduino boot/response messages so buffer stays clean
                    time.sleep(0.1)
                    if self.arduino_serial.in_waiting:
                        resp = self.arduino_serial.read(self.arduino_serial.in_waiting)
                        self.get_logger().info(f"Arduino boot response: {resp}")
    ...
    def write_to_arduino(self, data: bytes):
        ...
        try:
            if self.arduino_serial.is_open:
                # Drain any unread RX bytes first so the buffer never fills up
                if self.arduino_serial.in_waiting:
                    resp = self.arduino_serial.read(self.arduino_serial.in_waiting)
                    self.get_logger().debug(f"Arduino response: {resp}")

                self.get_logger().info(f"Writing to Arduino: {data}")
                self.arduino_serial.write(data)
                self.arduino_serial.flush()  # Ensure bytes are physically sent immediately
            else:
    ...
    def joy_callback(self, msg: Joy):
        ...
        if msg.buttons[0]:
            self.get_logger().info("Forward button pressed")
            self.write_to_arduino(b'F\n')  # Forward
        elif msg.buttons[1]:
            self.get_logger().info("Backward button pressed")
        ...
        velocity2 = msg.axes[5] * 5
        if msg.axes[4] < 0:
            velocity1 = msg.axes[4] * 55
            print(f"velocity1: {velocity1}")
        else:
            velocity1 = msg.axes[4] * 45
